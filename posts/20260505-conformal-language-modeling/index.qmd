...
- **The component-level filter is the more interesting half, and its scope matters.**
  ...
  - Retained components each come with a coverage guarantee but no calibrated probability, so the layer does not recover the posterior the background flagged as missing. It gives per-component admissibility instead, which addresses hallucination flagging but not candidate ranking.
...
- [Language Models with Conformal Factuality Guarantees](/posts/20260505-conformal-factuality/): the narrower specialization of this framework to factuality, with a side-by-side comparison of sub-claim back-off versus candidate sampling.
- [A Primer on Conformal Prediction](/posts/20260505-conformal-prediction-primer/): the finite-class recipe this paper extends to the combinatorial output space of generative LMs by replacing enumeration with a calibrated stopping rule.
- [Detecting Hallucinations in Large Language Models Using Semantic Entropy](/posts/20260505-semantic-entropy/): a sample-then-aggregate sibling sharing the same $N$-forward-pass backbone but returning a scalar abstention signal instead of a calibrated set.
- [Linguistic Calibration of Long-Form Generations](/posts/20260505-linguistic-calibration/): the same hallucination-control goal pursued by training calibrated surface form, which this paper's set-level guarantees leave untreated.
- [How to Make LLM Output More Trustworthy](/posts/20260505-llm-uncertainty-survey/): the survey that frames this paper as the first guarantee-based adaptation of conformal prediction to generative LMs.
