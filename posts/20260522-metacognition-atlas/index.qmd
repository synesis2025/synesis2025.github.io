...
- **Items.** 1,500 MMLU [[2]](#ref-2) questions split 250-per-domain across six a priori groupings (Applied/Professional, Factual recall, Formal reasoning, Humanities/Comprehension, Natural science, Social/Moral). Per-domain sampling drawn from a much larger MMLU pool:
...
Across 47,151 observations, every model with above-chance aggregate monitoring shows non-trivial domain-level variation. The figures break the picture down row by row.
...
- [LLMs Know When They Know, but Do Not Act on It: A Metacognitive Harness for Test-time Scaling](/posts/20260522-metacognitive-harness/): fits a per-model controller on confidence signals, taking half the step this atlas argues should run at per-(model, domain) resolution.
- [The Compliance Trap: How Structural Constraints Degrade Frontier AI Metacognition Under Adversarial Pressure](/posts/20260522-compliance-trap/): the adversarial counterpart, testing whether the family-level monitoring fingerprints here survive compliance-forcing pressure that the atlas never applies.
- [Language Models (Mostly) Know What They Know](/posts/20260504-kadavath-mostly-know/): the foundational self-evaluation result this atlas refines, moving from a single aggregate calibration number to per-domain variation.
- [Improving Metacognition and Uncertainty Communication in Language Models](/posts/20260525-metacognition-uncertainty-sft/): trains verbalized confidence the atlas measures, sharing the elicitation-as-hyperparameter premise that probe format shapes the score.
- [TRIAGE: Evaluating Prospective Metacognitive Control in LLMs Under Resource Constraints](/posts/20260523-triage-metacognitive-control/): the control-side companion to this monitoring-side atlas, scoring whether models act on self-assessment rather than just report it.
