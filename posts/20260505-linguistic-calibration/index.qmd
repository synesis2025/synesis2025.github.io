...
Evaluation uses both simulated readers (LMs conditioned on the generation) and real human readers, and factuality is scored at the claim level with a retrieval plus large-language-model (LLM) verifier pipeline in the style of FActScore [[3]](#ref-3).
...
- [Language Models with Conformal Factuality Guarantees](/posts/20260505-conformal-factuality/): the same Hashimoto group's post-hoc, guarantee-based take on long-form factuality, a counterpoint to this paper's train-the-prose approach.
- [Conformal Language Modeling](/posts/20260505-conformal-language-modeling/): the set-level and component-level guarantees this paper complements by treating the surface form of confidence those guarantees leave untreated.
- [SelfReflect: Can LLMs Communicate Their Internal Answer Distribution?](/posts/20260430-selfreflect-internal-distribution/): the sample-then-summarize regime this paper's summary-distillation step mirrors, and which it sidesteps by scoring reader forecasts rather than summary fidelity.
- [Improving Metacognition and Uncertainty Communication in Language Models](/posts/20260525-metacognition-uncertainty-sft/): a later attempt at the same goal of getting an LM to verbalize calibrated uncertainty in its own prose.
- [How to Make LLM Output More Trustworthy](/posts/20260505-llm-uncertainty-survey/): the survey that files this paper under behavioral calibration, distinct from the guarantee-based and detection-based branches.
