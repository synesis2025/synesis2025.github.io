---
title: "How Frontier LLMs Are Actually Trained and Served"
description: "Reiner Pope, MatX CEO and ex-Google TPU architect, spends two hours at a blackboard with Dwarkesh Patel deducing how labs train and serve frontier LLMs from a handful of equations, public API prices, and some chalk."
author: synesis
date: "2026-05-01"
image: cover.jpg
toc-depth: 4
...
![Thumbnail for the Dwarkesh Patel interview with Reiner Pope. [Watch on YouTube](https://www.youtube.com/watch?v=xmkSf5IS-zw)](cover.jpg){fig-align="center"}

In the [April 29 episode](https://www.youtube.com/watch?v=xmkSf5IS-zw) of the Dwarkesh Podcast, [Reiner Pope](https://reiner.org/) (CEO of chip startup [MatX](https://matx.com/), previously at Google working on software efficiency, compilers, and TPU architecture) walks through how GPT-5, Claude, and Gemini are trained and served. The format is a blackboard lecture, running a little over two hours. Pope derives the economics and architecture of frontier LLM deployment from first principles, using only published API prices, public benchmark numbers, and the arithmetic of memory bandwidth versus compute.

The two hours are dense enough that a flat transcript is hard to skim, so I've built an **[enriched transcript](#transcript) at the bottom of this post**: Dwarkesh's [raw gist](https://gist.github.com/dwarkeshsp/79100f0fdeed69d76241903bb0604dbe) sliced into 7 YouTube chapters and 59 fine-grained topic subsections (e.g., "solving for optimal batch size", "why prefill is 5× cheaper than decode", "reading off the 100× over-trained multiplier"), with hyperlinks added on first mention of every salient acronym, paper, model, product, and person. Pope's blackboard charts have been **redrawn as clean SVGs at the moments he sketches them on the board**, and the equations he writes out — the roofline pair, the optimal-batch derivation, the equal-cost-shares heuristic, the 100× Chinchilla ratio, the bytes-per-token solve from API pricing — have been **typeset as display math with notation glosses** so each new variable is defined on first use. The subsection outline is in the right-hand sidebar; use it to jump directly to a specific derivation instead of scrolling.
...
- [0:00:00 — How batch size affects token cost and speed](#ch-batch-size) ([YouTube](https://www.youtube.com/watch?v=xmkSf5IS-zw&t=0s))
- [0:31:59 — How MoE (mixture of experts) models are laid out across GPU racks](#ch-moe-racks) ([YouTube](https://www.youtube.com/watch?v=xmkSf5IS-zw&t=1919s))
- [0:47:02 — How pipeline parallelism spreads model layers across racks](#ch-pipeline-parallelism) ([YouTube](https://www.youtube.com/watch?v=xmkSf5IS-zw&t=2822s))
- [1:03:27 — Why Ilya said, "As we now know, pipelining is not wise."](#ch-ilya-pipelining) ([YouTube](https://www.youtube.com/watch?v=xmkSf5IS-zw&t=3807s))
- [1:18:49 — Because of RL (reinforcement learning), models may be 100× over-trained beyond Chinchilla-optimal](#ch-rl-overtrained) ([YouTube](https://www.youtube.com/watch?v=xmkSf5IS-zw&t=4729s))
- [1:32:52 — Deducing long context memory costs from API pricing](#ch-api-pricing) ([YouTube](https://www.youtube.com/watch?v=xmkSf5IS-zw&t=5572s))
- [2:03:52 — Convergent evolution between neural nets and cryptography](#ch-crypto) ([YouTube](https://www.youtube.com/watch?v=xmkSf5IS-zw&t=7432s))

## My Notes

- The headline claim of the lecture is that frontier architecture can be reverse-engineered from public API prices. API pricing has to cover real [FLOPs](https://en.wikipedia.org/wiki/Floating_point_operations_per_second) (floating-point operations per second) and real [HBM](https://en.wikipedia.org/wiki/High_Bandwidth_Memory) (high-bandwidth memory), so the per-token price floor constrains the model's active-parameter count, its [key-value (KV) cache](https://huggingface.co/docs/transformers/en/kv_cache) layout, and its serving batch size. Labs treat these numbers as secrets, but Pope's point is that the accounting identity between price and hardware forces the secret to leak through the invoice. The worked example is the jump to a higher price tier that [Gemini 3.1](https://blog.google/technology/google-deepmind/gemini-3/) (and other providers) apply above 200K context: Pope [solves for bytes-per-token at the compute/memory crossover](#gemini-3.1s-50-long-context-price-bump-solving-for-bytes-per-token) and gets ~2KB per token, which is consistent with a [reused-across-layers global context](#attention-heads-vs.-kv-heads-character-ai-style-layer-reuse) in the style [Character AI](https://research.character.ai/optimizing-inference/) has described. If the derivation holds, the "closed model" posture is more porous than the providers acknowledge.

- Chinchilla derived compute-optimal data-to-parameter ratios for pretraining. [RL](https://en.wikipedia.org/wiki/Reinforcement_learning) (reinforcement learning) post-training consumes compute the Chinchilla [[1]](#ref-1) curve does not price, and if the RL phase dominates total training FLOPs the pretraining ratio stops being the right optimization target. Pope's [back-of-envelope](#the-heuristic-equalize-training-cost-rl-cost-and-inference-cost): equalize pretraining cost, RL cost, and lifetime inference cost; plug in ~50M tokens/sec per model over a two-month lifetime; compare against ~100B active parameters. The pretraining token count [comes out ~100× above what Chinchilla would recommend](#reading-off-the-100-over-trained-multiplier). This extends the concern [Karpathy raised with Dwarkesh in October](/posts/20251018-karpathy-agi-decade-away/) about RL "through a straw": RL is sample-inefficient, and labs are now paying for that inefficiency with a compute budget that rivals the pretraining run.

- "Pipelining [[2]](#ref-2) is not wise" pushes back on a decade of distributed-training folklore. Pipeline parallelism was the standard answer for fitting models that exceed one rack's HBM. Pope's [argument for why it doesn't actually help inference](#why-pipelining-doesnt-save-kv-cache-memory-the-terms-cancel): splitting a model across P pipeline stages cuts the weight memory per rack by a factor of P, but the KV-cache memory per rack stays constant, because keeping every rack busy requires P microbatches in flight simultaneously. The per-rack improvement is zero. The alternative is either denser interconnect between racks (expensive, limited by physics) or MoE [[3]](#ref-3) (mixture of experts) partitioning that keeps each rack mostly self-contained (operationally harder, but bandwidth-kinder). The industry's [bet on MoE at the frontier](#what-labs-actually-do-maximum-expert-parallelism-minimum-pipelining) is the choice this argument predicts. See also [Sequential KV Cache Compression via Probabilistic Language Tries](/posts/20260422-kv-cache-compression/) for a related line on why the KV cache is now the dominant memory cost of serving.

- The real point of the crypto chapter is that cryptography and deep learning share the same primitives (differentiation, invertibility, high-dimensional mixing) and use them for opposite purposes. Both fields design large-parameter functions over high-dimensional spaces, and both are heavily constrained by the memory hierarchy; crypto hardware (AES, RSA, ECC accelerators) has driven chip design for thirty years, neural-network hardware for the last decade and a half. Pope draws three concrete bridges. [Adversarial attacks on image classifiers](#backdoors-and-adversarial-attacks-as-the-avalanche-property) are the [avalanche property](https://en.wikipedia.org/wiki/Avalanche_effect) of a cipher, a property ciphers *want* (small input change, total output change) and neural networks *don't*. [Differential cryptanalysis](https://en.wikipedia.org/wiki/Differential_cryptanalysis) works by differentiating a cipher with respect to its Boolean inputs, which is the same tool a neural net uses to train, applied here to break. And the [Feistel construction](https://en.wikipedia.org/wiki/Feistel_cipher) that makes ciphers invertible was [imported into neural networks](#feistel-networks-ported-into-neural-nets-as-revnets) as RevNets (2017) [[4]](#ref-4), where invertibility lets you rematerialize activations during the backward pass instead of caching them. That last trick is [the inverse of the KV-cache bargain](#trading-compute-for-memory-revnets-vs.-memory-for-compute-kv-cache): RevNets trade compute for memory; the KV cache is the same dial turned the other way.
...
What's here is [Dwarkesh's raw gist transcript](https://gist.github.com/dwarkeshsp/79100f0fdeed69d76241903bb0604dbe), reorganized into a reader-friendly form. The dialog itself is verbatim — no words added, cut, or reworded — but six layers of editorial structure have been added on top:
...
- **Inline hyperlinks.** On first mention of a salient acronym, concept, product, person, or company, a link has been added to the most authoritative source — the official product page, the person's homepage, Wikipedia. Research papers are cited by number and collected in the [References section](#references) at the end of the post.
- **Bracketed acronym expansions.** Where an acronym appears for the first time without being expanded in the original speech, its expansion is inserted in `[square brackets]` to mark the expansion as editorial rather than something the speaker actually said (e.g. `TPU [tensor processing unit]`).
- **Reproduced blackboard charts.** Each of Pope's seven schematic plots — latency vs. batch size, cost-per-token vs. batch, the 20 ms train timeline, the pipeline bubble (inference and training panels), cost vs. context length, cost vs. length-of-pass, and FLOPs vs. context length — has been redrawn as a clean SVG and embedded at the moment in the dialog where Pope sketches it. The axes are unitless because the original blackboard curves are unitless: they illustrate shape, slope, and crossover points, not calibrated numbers.
...
Then, number of active parameters. If I look at, for example, a [DeepSeek](https://huggingface.co/deepseek-ai) model, the [DeepSeek V3](https://huggingface.co/deepseek-ai/DeepSeek-V3) model has about 37 billion active parameters, and 700 billion total parameters. We're focusing on just the ones that are active for a single AI token.
...
**Reiner Pope:** When I do a forward pass… Let me draw how the [autoregressive](https://en.wikipedia.org/wiki/Autoregressive_model) inference works. This is during decode. If I have a bunch of text tokens… I'm drawing a tensor because ultimately the tokens are represented as a tensor in some embedding dimension. In this direction, I have the sequence length.
...
![Three curves and the envelope: compute time grows linearly with batch size, the KV-fetch term grows linearly too, and the weight-fetch term sits as a constant offset. The black line is the wallclock floor — $\max(t_\text{compute},\, t_\text{mem})$. The balance point is where the slopes cross.](fig-latency-vs-batch.svg){fig-align="center"}
...
::: {.callout-note}
**Editor's note.** The GPU rents two scarce resources per second — FLOPs and HBM bandwidth — and wallclock latency is `max(t_compute, t_memory)`. Whichever is slower dictates the runtime; whichever is faster sits idle and is wasted rental. Equating them saturates both axes simultaneously, which is also where the cost-per-token curve bottoms out. The equality is the equation Reiner solves [below](#solving-for-optimal-batch-size-b-300-sparsity) to derive the B ≈ 300 × sparsity rule of thumb.
:::


#### MFU inside vs. outside the Goldilocks zone

**Dwarkesh Patel:** This is a very simple algebra problem, but suppose the optimal is 100K context length, and you go to 200K context length. Does your [MFU](https://github.com/stas00/ml-engineering/blob/master/training/performance/README.md#mfu-vs-hfu) [model FLOPs utilization] go down to 50%? Does it have a humongous impact on MFU to be slightly outside of the optimal context length range, the Goldilocks zone?
...
**Dwarkesh Patel:** Got it. Is sparse attention [[5]](#ref-5) what everybody uses in practice?
...
![Same three curves divided by batch size $B$. Compute and KV become flat (since both are linear in $B$); weight fetch becomes a $1/B$ hyperbola. The cost-per-token envelope falls steeply at small $B$ as the weight fetch amortizes, then floors at the compute-bound rate.](fig-cost-vs-batch.svg){fig-align="center"}
...
$$
\frac{N_\text{total}}{\text{BW}_\text{mem}} \;=\; \frac{B \cdot N_\text{active}}{\text{FLOPs}}
\quad\Longleftrightarrow\quad
\frac{\text{FLOPs}}{\text{BW}_\text{mem}} \;=\; B \cdot \frac{N_\text{active}}{N_\text{total}}
$$
...
$$
B \;\gtrsim\; 300 \times \text{sparsity},
\qquad \text{sparsity} = \frac{N_\text{total}}{N_\text{active}}
$$
...
![Inference runs as a regular train schedule: a new batch leaves every 20 ms regardless of fill. A request arriving just after a departure waits up to 20 ms for the next train, then 20 ms more to be processed — worst-case latency 40 ms.](fig-20ms-train.svg){fig-align="center"}
...
**Reiner Pope:** For example, on the [Rubin](https://www.nvidia.com/en-us/data-center/vera-rubin/) generation, it is something like 288 gigabytes divided by 20 terabytes per second. This comes out to about 15 milliseconds.
...
**Reiner Pope:** This paper is "Unified Scaling Laws for Routed Language Models" [[9]](#ref-9). It's a somewhat old paper by this stage, but one of the things they looked at is if I keep increasing sparsity, what is the model quality impact? This answer is very sensitive to the actual choice of mixture of experts. Mixture of experts [[3]](#ref-3) has been around for a really long time, maybe even back in 2017, but the techniques have changed a lot. DeepSeek's mixture of experts was a big change in how it worked. There have been older papers, like "GShard" [[10]](#ref-10) and "Switch Transformer" [[11]](#ref-11). The actual empirical results are going to depend on all of that.
...
### [0:31:59 — How MoE models are laid out across GPU racks](https://www.youtube.com/watch?v=xmkSf5IS-zw&t=1919s) {#ch-moe-racks}
...
The router will make a decision of which experts it's going to route to, and it will be a small fraction of them, maybe 1 in 32. Maybe it will make a decision to route to this one, maybe this one, and maybe this one. Each expert itself is a normal MLP [[multi-layer perceptron](https://en.wikipedia.org/wiki/Multilayer_perceptron)]. It has an up projection and then a down projection with a nonlinearity in between.

::: {.callout-note}
**Editor's note.** Modern MoE routes each token to multiple experts — top-k with k > 1. Mixtral uses top-2 of 8; DeepSeek-V3 uses top-8 of 256 plus a shared expert that always fires. Pope's "1 in 32" is the fraction of experts active, not the literal count, and his "maybe this one, maybe this one, and maybe this one" hints at the top-k picture. The "32 out of 256" DeepSeek figure he cites [earlier in the transcript](#solving-for-optimal-batch-size-b-300-sparsity) is this in practice. Activating multiple fine-grained experts per token and weighted-summing their outputs is part of why newer MoE designs beat the Switch-Transformer-era `64× for 4×` ratio.
:::
...
What I want to talk through is how this is mapped to a GPU rack and what this means for communication, because I think this will start to show some of the limits of how sparse we can go. The standard practice here, and it is the best solution, is to use expert parallelism [[10]](#ref-10). That means different experts go on different GPUs. If we take something like a DeepSeek model, they have 256 experts. Let's say we want to run that on a Blackwell rack. There are 72 GPUs.
...
**Dwarkesh Patel:** What has changed that has allowed Nvidia to go from [Hopper](https://www.nvidia.com/en-us/data-center/technologies/hopper-architecture/), which was 8, then Blackwell is 72, and now Rubin will be... is it 500 something?
...
**Dwarkesh Patel:** When was [GPT-4](https://openai.com/index/gpt-4/) released again? Was it 2022 or 2023?
...
### [0:47:02 — How pipeline parallelism spreads model layers across racks](https://www.youtube.com/watch?v=xmkSf5IS-zw&t=2822s) {#ch-pipeline-parallelism}
...
There are other kinds of parallelism besides expert parallelism, which we just showed here. In the literature is tensor parallelism [[12]](#ref-12). With the trend towards smaller experts, this has become much less relevant, so we can ignore that. But the other two things we have available are data parallelism [[13]](#ref-13) and pipeline parallelism. They can be a much better fit for using multiple racks.
...
$$
\frac{t_\text{scale-up}}{t_\text{scale-out}}
\;=\;
\underbrace{\tfrac{1}{8}}_\text{bandwidth ratio}
\cdot \underbrace{n_\text{activated experts}}_\text{routing fan-out}
\cdot \underbrace{2}_\text{up + down}
\cdot \underbrace{n_\text{layers/stage}}_\text{reuse per hop}
\;\geq\; 1
$$
...
**Dwarkesh Patel:** So there's [a talk](/posts/20241213-sutskever-pretraining-over/#ch-parallelization) by [Ilya](https://en.wikipedia.org/wiki/Ilya_Sutskever) where he says, "Today we know not to do pipeline parallelism." And [Horace He](https://horace.io/) gave my friends and me… I hate that it sounds like a Dr. Seuss quote. But he gave us a lecture on these different kinds of parallelisms. He said the problem with pipeline parallelism is that, other than the bubbles, it creates these architectural constraints. [Kimi](https://moonshotai.github.io/Kimi-K2/), for example, has these residuals where attention attends to layers a few back, so it becomes hard to implement in this way.
...
**Dwarkesh Patel:** The opposite connotation to this… Before this interview, I was chatting with Axel, who's a GPU performance engineer at [Jane Street](https://www.janestreet.com/). He was explaining that to do pipelining, you have to do micro-batches rather than full batches. If you do micro-batches, then you're by definition not able to amortize loading the weights across all the users or all the sequences. The positive connotation of that is you don't have to use as much memory. The negative connotation is that we can't amortize loading the weights across all those users. Maybe it's worth explaining why you have to do micro-batches.
...
![Pipeline schedule, two regimes. **Inference** (left): once filled, the pipeline runs back-to-back with no idle time after the initial bubble. **Training** (right): a hard stop separates forward (blue) and backward (orange) passes — the backward pass starts at the deepest rack and propagates back, mirroring the forward staircase.](fig-pipeline-bubble.svg){fig-align="center"}
...
$$
c_\text{mem} \;=\; \frac{N_\text{total}}{E \cdot P} \;+\; \frac{(P \cdot b) \cdot L_\text{ctx} \cdot \text{bytes/tok}}{E \cdot P}
\;=\; \underbrace{\frac{N_\text{total}}{E \cdot P}}_\text{shrinks with } P \;+\; \underbrace{\frac{b \cdot L_\text{ctx} \cdot \text{bytes/tok}}{E}}_\text{independent of } P
$$
...
$$
\text{BW}_\text{mem,\, effective} \;=\; (\text{scale-up size}) \times \text{BW}_\text{per-GPU},
\qquad
t_\text{mem} \;=\; \frac{N_\text{total}}{\text{BW}_\text{mem,\, effective}}
$$
...
#### Solving for equal training / RL / inference cost shares

**Dwarkesh Patel:** Sorry, I think the way I said it was super garbled. Just for the audience, forward plus backwards per parameter is 6. Forward alone is 2. That's why RL, where you're definitely going to generate all the trajectories but you might or might not train all the trajectories, is 2 to 6.
...
**Dwarkesh Patel:** I'm not sure I understand the intuition for that. Another naive model could have been that RL plus pre-training would be 50% and inference would be 50%.

**Reiner Pope:** That's also a valid answer. Because this is heuristic, I can't really argue for one versus the other. They don't differ by that much. Thirty-three versus twenty-five is only a small factor off.
...
$$
D_\text{pre} \;+\; \alpha \cdot D_\text{RL} \;=\; \beta \cdot D_\text{inf},
\qquad \alpha \approx \tfrac{1}{10},\;\; \beta \approx \tfrac{1}{10}
$$

::: {.callout-note}
**Notation.** $\beta$ rescales the inference term to match Pope's "all three roughly equal" budget split (each ~1/3 of total compute). The 1/10 falls out of $\tfrac{1}{3} \times 2/6 \times \text{(efficiency factor)}$. Pope flips signs once mid-derivation and lands on the cleaner ratio $D_\text{pre} : D_\text{RL} : D_\text{inf} \approx 1 : 1 : 1$ — see the editor's note above.
:::

**Dwarkesh Patel:** If both of them are one in ten, that kind of implies that there's never a backward pass on RL?

**Reiner Pope:** Yeah. Okay, we can make this 2/10. Make it a bit bigger. Just write it out once more, this is 2/10, this is 1/10.
...
I guess we didn't do the equivalence between pre-training and RL, so we'll do that here. Data in pre-training should be equal to 2/10 data in RL for them to be cost equivalent. Sorry, 1/10. I got it backwards. We pay more cost when it's inefficient, so this needs to be 1/10.

::: {.callout-note}
**Editor's note.** Pope reverses direction mid-derivation and rounds the final ratio to "1.5 and 1" to make the three cost terms equalize cleanly. The 100× over-trained multiplier that falls out inherits the wide error bars he acknowledges a few paragraphs later.
:::
 Tracing this back… This thing ends up actually being, as written here… This is like 1.5, and this is one.

**Dwarkesh Patel:** Billions of dollars worth of compute just flowed in the other direction.

**Reiner Pope:** Right? I think if you do it with a spreadsheet and actually model it out, you might notice when the money’s going down the drain. All of these end up being close in, as modeled here. This 30% may have been a little bit too generous. So let's say something like 1.5 here, and leave this as a one here.
...
::: {.callout-note}
**Notation.** $D_\text{Chinchilla}$ = the pre-training token count the Chinchilla scaling law [[1]](#ref-1) recommends for a model of given parameter count, at the compute-optimal point. The 20× heuristic is the empirical fit: optimal pre-training tokens ≈ 20 × parameters. For an active-parameter count of 100 B, that gives 2 trillion tokens — versus the ~200 trillion frontier labs are reportedly using.
:::
...
### [1:32:52 — Deducing long context memory costs from API pricing](https://www.youtube.com/watch?v=xmkSf5IS-zw&t=5572s) {#ch-api-pricing}

#### Gemini 3.1's 50% long-context price bump: solving for bytes per token

![Cost per token as a function of context length: compute is roughly flat, weight fetch is a constant offset, and the KV-fetch term grows linearly with $L$. The black envelope inflects around 200 K tokens — a natural place to set a tier boundary, which is why Gemini's price bumps at exactly that length.](fig-cost-vs-context.svg){fig-align="center"}
...
First, with longer context, [Gemini 3.1](https://blog.google/technology/google-deepmind/gemini-3/) is 50% more expensive if you go over 200k tokens than if you're below 200k tokens. At a high level, I understand why that might be, but why specifically 50%?
...
$$
\text{bytes/tok}
\;=\; \frac{N_\text{active}}{(\text{FLOPs}/\text{BW}_\text{mem}) \cdot L_\text{ctx}}
\;\approx\; \frac{10^{11}}{300 \cdot 2\times 10^{5}}
\;\approx\; 1{,}667\ \text{bytes}
$$
...
**Reiner Pope:** That is plausible, actually. You said around two kilobytes. Let's just do a sanity check for what this could be. There are two mechanisms that people do attention with a small number of bytes per token. One is dense attention with a lot of reuse across layers. [Character AI](https://character.ai/) has a [blog post](https://research.character.ai/optimizing-inference/) talking about that, alternating long and short context. In the Character AI kind of model, which also showed up in the [Gemma](https://ai.google.dev/gemma) models, the global context—which is really what we're talking about here—was shared across all the layers.

::: {.callout-note}
**Editor's note.** Character AI's [inference blog](https://research.character.ai/optimizing-inference/) describes cross-layer KV sharing plus interleaved local/global attention. Gemma uses alternating local/global attention per layer but does not literally share one global KV across all layers the way Character AI does. The two designs are related but not identical.
:::
...
**Dwarkesh Patel:** Okay, suppose we're here. You will have calculated all of this previously. So just the KV of everything that came before. But what is the memory cost of this? Well, the memory bandwidth cost of this. If you're doing flash attention [[15]](#ref-15), it would—
...
![Cost per token versus length of the pass. Compute per token is flat; memory per token falls as $1/L_\text{pass}$ because the bandwidth cost amortizes over the whole pass. Decode ($L_\text{pass}=1$) sits on the memory-bound branch, prefill (large $L_\text{pass}$) sits on the compute-bound floor — explaining the ~5× cheaper input pricing.](fig-cost-vs-lenpass.svg){fig-align="center"}
...
![FLOPs per token vs. context length on a log scale. The weight-matmul term is constant; the KV-attention term is linear in $L$ (so total cost is quadratic in $L$). The slope is so shallow that the linear term only catches up around millions of tokens — for typical 100 K–1 M contexts, total FLOPs/token is well-approximated as flat.](fig-flops-vs-context.svg){fig-align="center"}
...
### [2:03:52 — Convergent evolution between neural nets and cryptography](https://www.youtube.com/watch?v=xmkSf5IS-zw&t=7432s) {#ch-crypto}
...
This has actually been ported over into neural nets. There's a 2017 paper called RevNets [[4]](#ref-4), reversible networks. What it does is make the entire network invertible. You can apply it to any network, like a transformer network. I do a forwards pass, but then I can run the entire pass backwards as well. The whole neural network is invertible with exactly this construction.
...
- [Dwarkesh Podcast — episode with Reiner Pope](https://www.youtube.com/watch?v=xmkSf5IS-zw)
- [Raw transcript gist](https://gist.github.com/dwarkeshsp/79100f0fdeed69d76241903bb0604dbe)

**People**

- [Reiner Pope](https://reiner.org/)
...
- [MatX](https://matx.com/)
- [Character AI](https://character.ai/) and its [inference-optimization blog post](https://research.character.ai/optimizing-inference/)
- [Jane Street](https://www.janestreet.com/)
- [SemiAnalysis](https://www.semianalysis.com/)
- [DeepSeek on Hugging Face](https://huggingface.co/deepseek-ai), [DeepSeek V3 model card](https://huggingface.co/deepseek-ai/DeepSeek-V3)
- [Gemini 3 announcement](https://blog.google/technology/google-deepmind/gemini-3/)
- [Gemma](https://ai.google.dev/gemma)
- [GPT-4 release page](https://openai.com/index/gpt-4/)
- [Kimi K2](https://moonshotai.github.io/Kimi-K2/)

**Hardware**

- [Nvidia Blackwell NVL72](https://www.nvidia.com/en-us/data-center/gb200-nvl72/)
- [Nvidia Vera Rubin](https://www.nvidia.com/en-us/data-center/vera-rubin/)
...
- [FP4 minifloat](https://en.wikipedia.org/wiki/Minifloat#FP4)
- [FLOPs](https://en.wikipedia.org/wiki/Floating_point_operations_per_second)

**Background concepts**

- [Roofline model](https://en.wikipedia.org/wiki/Roofline_model)
- [KV cache (Hugging Face Transformers docs)](https://huggingface.co/docs/transformers/en/kv_cache)
- [Autoregressive model](https://en.wikipedia.org/wiki/Autoregressive_model)
- [Reinforcement learning](https://en.wikipedia.org/wiki/Reinforcement_learning)
- [MFU vs. HFU (Stas Bekman's ML-engineering notes)](https://github.com/stas00/ml-engineering/blob/master/training/performance/README.md#mfu-vs-hfu)
...
1. <a id="ref-1"></a>Hoffmann, Jordan, et al. "Training Compute-Optimal Large Language Models." *arXiv preprint*, 2022. <https://arxiv.org/abs/2203.15556>
2. <a id="ref-2"></a>Huang, Yanping, et al. "GPipe: Efficient Training of Giant Neural Networks Using Pipeline Parallelism." *arXiv preprint*, 2018. <https://arxiv.org/abs/1811.06965>
3. <a id="ref-3"></a>Shazeer, Noam, et al. "Outrageously Large Neural Networks: The Sparsely-Gated Mixture-of-Experts Layer." *arXiv preprint*, 2017. <https://arxiv.org/abs/1701.06538>
4. <a id="ref-4"></a>Gomez, Aidan N., et al. "The Reversible Residual Network: Backpropagation Without Storing Activations." *arXiv preprint*, 2017. <https://arxiv.org/abs/1707.04585>
5. <a id="ref-5"></a>Yuan, Jingyang, et al. "Native Sparse Attention: Hardware-Aligned and Natively Trainable Sparse Attention." *arXiv preprint*, 2025. <https://arxiv.org/abs/2502.11089>
6. <a id="ref-6"></a>Pope, Reiner, et al. "Efficiently Scaling Transformer Inference." *arXiv preprint*, 2022. <https://arxiv.org/abs/2211.05102>
7. <a id="ref-7"></a>Austin, Jacob, et al. "How to Scale Your Model: A Systems View of LLMs on TPUs." *Google DeepMind*, 2025. <https://jax-ml.github.io/scaling-book/>
8. <a id="ref-8"></a>Horowitz, Mark. "Computing's Energy Problem (and What We Can Do About It)." *IEEE International Solid-State Circuits Conference (ISSCC)*, 2014. <https://ieeexplore.ieee.org/document/6757323>
9. <a id="ref-9"></a>Clark, Aidan, et al. "Unified Scaling Laws for Routed Language Models." *arXiv preprint*, 2022. <https://arxiv.org/abs/2202.01169>
10. <a id="ref-10"></a>Lepikhin, Dmitry, et al. "GShard: Scaling Giant Models with Conditional Computation and Automatic Sharding." *arXiv preprint*, 2020. <https://arxiv.org/abs/2006.16668>
11. <a id="ref-11"></a>Fedus, William, Barret Zoph, and Noam Shazeer. "Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity." *arXiv preprint*, 2021. <https://arxiv.org/abs/2101.03961>
12. <a id="ref-12"></a>Shoeybi, Mohammad, et al. "Megatron-LM: Training Multi-Billion Parameter Language Models Using Model Parallelism." *arXiv preprint*, 2019. <https://arxiv.org/abs/1909.08053>
13. <a id="ref-13"></a>Krizhevsky, Alex. "One weird trick for parallelizing convolutional neural networks." *arXiv preprint*, 2014. <https://arxiv.org/abs/1404.5997>
14. <a id="ref-14"></a>Kaplan, Jared, et al. "Scaling Laws for Neural Language Models." *arXiv preprint*, 2020. <https://arxiv.org/abs/2001.08361>
15. <a id="ref-15"></a>Dao, Tri, et al. "FlashAttention: Fast and Memory-Efficient Exact Attention with IO-Awareness." *arXiv preprint*, 2022. <https://arxiv.org/abs/2205.14135>

## Related Posts

- [Sequential KV Cache Compression via Probabilistic Language Tries](/posts/20260422-kv-cache-compression/): a direct response to the lecture's central constraint, that the KV cache is the dominant and unamortizable memory cost of serving, by compressing the cache as a sequence.
- [The Inference Shift](/posts/20260511-the-inference-shift/): the same memory-hierarchy-first reading of serving economics, arguing that agentic inference is a distinct workload that reshapes the compute stack the lecture prices out.
- [The Math Behind the Cost of AI Agents](/posts/20260513-the-math-behind-the-cost-of-ai-agents/): the same price-it-out-from-first-principles method, deriving why agent token costs grow quadratically without caching and fitting the curves to real traces.
- [Karpathy: AGI Is Still a Decade Away](/posts/20251018-karpathy-agi-decade-away/): the "RL through a straw" concern the lecture extends, where the 100×-over-trained estimate quantifies how much compute the RL phase now consumes.
- [Sutskever at NeurIPS 2024: Pre-Training Era Is Over](/posts/20241213-sutskever-pretraining-over/): the source of Ilya's "pipelining is not wise" line, which the lecture derives mechanically by showing the per-rack KV-cache terms cancel.
